...
{
  QgsDecorationLayoutExtentDialog dlg( *this );
  dlg.exec();
}
...
void QgsDecorationLayoutExtent::render( const QgsMapSettings &mapSettings, QgsRenderContext &context )
{
  ...
  for ( QgsPrintLayout *layout : layouts )
  {
    ...
    for ( const QgsLayoutItemMap *map : qgis::as_const( maps ) )
    {
      ...
      if ( mLabelExtents )
      {
        QgsTextRenderer::drawText( labelPoint, ( map->mapRotation() - mapSettings.rotation() ) * M_PI / 180.0, QgsTextRenderer::AlignRight, QStringList() << tr( "%1: %2" ).arg( layout->name(), map->displayName() ),
                                             context, mTextFormat );
      }
    }
  }
